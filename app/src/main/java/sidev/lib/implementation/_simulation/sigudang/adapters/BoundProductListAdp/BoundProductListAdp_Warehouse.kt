...
import com.sigudang.android.utilities.view.component.BoundProductItemComp_old_Warehouse

class BoundProductListAdp_Warehouse(ctx: Context, dataList: ArrayList<BoundProduct>?)
    : BoundProductListAdp_Abs<BoundProductItemComp_old_Warehouse>(ctx, dataList){
    ...
    init{
        isPackagingVisible= true
        isSendVisible_Warehouse= true
        isAmountOverallFixed= true
    }

    override fun bindVH(vh: SimpleViewHolder, pos: Int, data: BoundProduct) {
        super.bindVH(vh, pos, data)
        val comp= getViewComp(pos)
        comp?.isReceiptVisible= isReceiptVisible
        comp?.setTrackText(trackTxtUpper, trackTxtLower)
    }

    override fun createViewComp(c: Context, view: View?): BoundProductItemComp_old_Warehouse {
        val comp= BoundProductItemComp_old_Warehouse(c, view)
        comp.isReceiptVisible= isReceiptVisible
        return comp
    }
}
