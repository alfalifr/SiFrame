...
val ARTIFACT_ID= project.name//.toLowerCase().also { println(it) } //"SiFrame"
//val ARTIFACT_ID_LOWER= project.name.toLowerCase().also { println(it) } //"SiFrame"
...
//val aarArtifactLower= "$aarDir/$ARTIFACT_ID_LOWER-release.aar"
/*
File(aarArtifact).renameTo(File(aarArtifactLower)).also {
    println("$ARTIFACT_ID renameTo $ARTIFACT_ID_LOWER it = $it")
}
 */
...
fun MavenPublication.deleteGithubPkgVersion(){
    val urlStr= "https://api.github.com/orgs/alfalifr/packages/maven/$groupId.$artifactId/versions/$version"
    URL(urlStr).openConnection().apply {
        this as `java.net`.HttpURLConnection
        requestMethod = "DELETE"
        headerFields["Authorization"]= listOf("token $githubApiKey")
...
// */
    ...
    val conn2 = URL(urlStr).openConnection() as `java.net`.HttpURLConnection
